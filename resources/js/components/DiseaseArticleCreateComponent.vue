...
export default {
    data: function () {
        return {
            diseaseArticleCreate: {}
        }
    },
    methods: {
        submit() { // submitメソッドで、diseaseArticleCreateの中身を記事登録APIにPOST送信する処理
            axios.post('/api/diseaseArticles', this.diseaseArticleCreate)
                .then((res) => {
                    this.$router.push({name: 'disease.article.list'}); // 登録後、記事一覧ページにリダイレクト
                });
        }
    }
}
